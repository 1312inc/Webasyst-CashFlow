...
class cashTransactionDtoAssembler
{
     ...
     * @throws waException
     */
    public function findByDatesAndAccount(DateTime $startDate, DateTime $endDate, array $accounts = [])
    {
        /** @var cashTransactionModel $model */
        $model = cash()->getModel(cashTransaction::class);

        $accountDtos = [];
        foreach (cash()->getEntityRepository(cashAccount::class)->findAllActive() as $account) {
            $accountDtos[$account->getId()] = cashDtoFromEntityFactory::fromEntity(cashAccountDto::class, $account);
            $accountDtos[$account->getId()]->currency = cashCurrencyVO::fromWaCurrency($account->getCurrency());
        }
