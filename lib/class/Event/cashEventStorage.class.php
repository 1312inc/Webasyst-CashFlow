<?php

/**
 * Class cashEventStorage
 */
final class cashEventStorage
{
    public const WA_BACKEND_SIDEBAR              = 'backend_sidebar';
    public const WA_BACKEND_TRANSACTION_DIALOG   = 'backend_transaction_dialog';
    public const WA_BACKEND_IMPORT_FILE_UPLOADED = 'backend_import.file_uploaded';
    public const WA_BACKEND_TRANSACTIONS_EXPORT  = 'backend_transactions_export';
    public const WA_REPORTS_MENU_ITEM = 'backend_reports_menu_item';
    public const WA_REPORTS_HANDLE = 'backend_reports_handle';
    public const WA_IMPORTS_MENU_ITEM = 'backend_imports_menu_item';
    public const WA_IMPORTS_HANDLE = 'backend_imports_handle';

    public const SETTINGS_SAVE = 'settings_save';
    public const TRANSACTION_PAGE_PREEXECUTE = 'transaction_page_preexecute';
    public const ON_COUNT = 'on_count';
    public const ACCOUNT_ARCHIVE = 'account_archive';

    public const API_TRANSACTION_RESPONSE_EXTERNAL_DATA = 'api_transaction_response_external_data';
    public const API_TRANSACTION_BEFORE_RESPONSE = 'api_transaction_before_response';
}
