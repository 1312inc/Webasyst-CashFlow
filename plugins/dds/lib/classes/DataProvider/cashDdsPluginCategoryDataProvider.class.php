...
final class cashDdsPluginCategoryDataProvider implements cashDdsPluginDataProviderInterface
{
    /**
    ...
    /**
     * cashDdsPluginCategoryDataProvider constructor.
     *
     * @throws waException
     */
    ...
    group by id, ca.currency, MONTH(ct.date)",
        ...
        // incomes
        // total
        $statData[] = new cashDdsPluginStatDto(
    ...
    private function createDdsStatDto(cashCategory $category, $rawData): cashDdsPluginStatDto
    {
        return new cashDdsPluginStatDto(
            new cashDdsPluginEntity(
                $category->getName(),
                $category->getId(),
                $category->isExpense(),
    ...
    private function initAmountPerPeriod(cashDdsPluginPeriod $period, &$rawData, $datumId, $datumCurrency, $id): void
    {
    ...
    private function initTotalAmountPerPeriod(cashDdsPluginPeriod $period, &$rawData, $datum, $type): void
    {
        foreach ($period->getGrouping() as $groupingDto) {
            $initVals = [
                'id' => $type,
